  ...
  if (!route.covered) {
    return (
      <div className="flex items-center gap-3">
        <span className="font-mono text-[13px] font-medium">{route.currency}</span>
        <span className="flex items-center gap-1.5 text-[12.5px] text-[var(--danger)]">
          <TriangleAlert className="size-3.5" aria-hidden />
          No enabled provider supports this currency.
        </span>
      </div>
    );
  }
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-center gap-3">
      ...
      <label className="flex items-center gap-2 text-[12.5px]">
        ...
        <select
          name="primary"
          defaultValue={route.primary ?? route.available[0]}
